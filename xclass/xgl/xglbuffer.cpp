...
void XGLBuffer::Load(std::vector<XGLVertexAttributes> va, std::vector<XGLIndex> ib){
    ...
    // vertex normals
    glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(XGLVertexAttributes), reinterpret_cast<void *>(sizeof(XGLVertex)));
    GL_CHECK("glVertexAttribPointer() failed");
    // vertex texCoords
    glVertexAttribPointer(2, 2, GL_FLOAT, GL_TRUE, sizeof(XGLVertexAttributes), reinterpret_cast<void *>(sizeof(XGLVertex) + sizeof(XGLNormal)));
    GL_CHECK("glVertexAttribPointer() failed");
    // vertex colors
