...
// glGetString(GL_EXTENSIONS) is deprecated in OpenGL 3.whatever
// so this is a cheap-ass replacement for use here.
// probably sub-optimal, but SOIL only looks once so fuck it,
static int soilIsSupported(char *ext){
    GLint numExtensions;
...
int query_NPOT_capability( void )
{
	...
	if( has_NPOT_capability == SOIL_CAPABILITY_UNKNOWN )
	{
        ...
        if (!soilIsSupported("GL_ARB_texture_non_power_of_two"))
		{
...
int query_tex_rectangle_capability( void )
{
	...
	{
		/*	we haven't yet checked for the capability, do so	*/
        if (!soilIsSupported("GL_ARB_texture_rectangle") &&
            !soilIsSupported("GL_EXT_texture_rectangle") &&
            !soilIsSupported("GL_NV_texture_rectangle"))
		{
			/*	not there, flag the failure	*/
...
int query_cubemap_capability( void )
{
	...
	if( has_cubemap_capability == SOIL_CAPABILITY_UNKNOWN )
	{
		/*	we haven't yet checked for the capability, do so	*/
		if(
            (!soilIsSupported("GL_ARB_texture_cube_map"))
		&&
        (!soilIsSupported("GL_EXT_texture_cube_map"))
			)
		{
			/*	not there, flag the failure	*/
...
int query_DXT_capability( void )
{
	...
	{
		/*	we haven't yet checked for the capability, do so	*/
        if (!soilIsSupported("GL_EXT_texture_compression_s3tc"))
		{
			/*	not there, flag the failure	*/
