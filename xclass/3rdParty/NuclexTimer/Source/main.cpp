...
int main() {
  ...
  steppedTimer.SetStepFrequency(6); // 6 Hz
  ...
  for(;;) {
    ...
    while(steppedTimer.TryAdvance(gameTime)) {
      cout << "Time stepped " << gameTime.RealWorldDeltaUs
           << " microseconds (step #" << stepIndex++ << ")" << endl;
    }
    ...
    cout << "Running for " << gameTime.RealWorldTotalUs
         << " microseconds" << endl;
